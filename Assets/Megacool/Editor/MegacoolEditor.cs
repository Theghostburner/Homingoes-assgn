...
class Startup {
    static Startup() {
        ...
        if (apiVersion < MegacoolEditor.MIN_API_VERSION) {
            string minApiError = "The application's minimum API version (" + apiVersion +
                ") is less than the required API version by Megacool (" +
                MegacoolEditor.MIN_API_VERSION + ")";

            if (EditorUtility.DisplayDialog("Megacool API", minApiError,
                "Change to " + MegacoolEditor.MIN_API_VERSION, "Cancel")) {
                    PlayerSettings.Android.minSdkVersion = MegacoolEditor.MIN_API_VERSION;
            }
        }
...
public class MegacoolEditor : Editor {
    ...
    // API levels lower than 19 are deprecated in 2019.3 and up
    public static AndroidSdkVersions MIN_API_VERSION = AndroidSdkVersions.AndroidApiLevel19;
#else
    public static AndroidSdkVersions MIN_API_VERSION = AndroidSdkVersions.AndroidApiLevel16;
#endif
    ...
    public override void OnInspectorGUI() {
        ...
        addHelpBox("The keys above are required for the platforms you develop for. You can find " +
            "them on your Megacool dashboard at https://dashboard.megacool.co");
    ...
    public static void OpenDocs() {
        Application.OpenURL("https://docs.megacool.co/sdk/unity/5.0.4/");
    }
